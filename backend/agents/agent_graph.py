...
from agents.evaluator import evaluate_and_print
import time
...
class ResearchState(TypedDict):
    ...
    _on_step: Optional[Any]   # callback: fn(step_number, label)
...
def reason_node(state: ResearchState) -> ResearchState:
    ...
    reasoning = generate_reasoning(state["topic"], critic_feedback=feedback)
...
def hypothesize_node(state: ResearchState) -> ResearchState:
    ...
    result = generate_hypotheses(state["topic"], state["rag_context"], state["papers"], critic_feedback=feedback)
    time.sleep(_CALL_DELAY)
...
def formalize_node(state: ResearchState) -> ResearchState:
    ...
    math = generate_math_formulation(state["topic"], summary, critic_feedback=feedback)
    time.sleep(_CALL_DELAY)
...
def roadmap_node(state: ResearchState) -> ResearchState:
    ...
    roadmap = generate_roadmap(state["topic"], subtopics, difficulty, critic_feedback=feedback)
    time.sleep(_CALL_DELAY)
    return {**state, "research_roadmap": roadmap}
...
def critic_node(state: ResearchState) -> ResearchState:
    ...
    return {
        **state,
        ...
        "_retry_nodes": critique.get("retry_nodes", []),
        "_approved": critique.get("approved", True),
    }
...
def critic_router(state: ResearchState) -> str:
    ...
    if approved or retry_count > MAX_RETRIES or not retry_nodes:
        print(f"[Critic] Approved. Scores: {state.get('critic_scores')}")
        return "end"

    node_order = ["reason", "hypothesize", "formalize", "roadmap"]
    for node in node_order:
        if node in retry_nodes:
            print(f"[Critic] Retrying '{node}'...")
            return node
...
def run_pipeline(topic: str, on_step: Optional[Callable] = None) -> dict:
    ...
    result = {
        ...
        "roadmap": final_state["research_roadmap"],
        "critic_scores": final_state.get("critic_scores"),
    }
    try:
        evaluate_and_print(result)
